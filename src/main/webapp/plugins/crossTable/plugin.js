...
var crossTable = {
    table: function (args) {
        ...
        var PaginationDom = "<div class='" + p_class + "'><div class='optionNum'><span>" + brhcTranslate("CROSS_TABLE.SHOW") + "</span>" + optionDom + "<span>" + brhcTranslate("CROSS_TABLE.ENTRIES") + "</span></div><div class='page'><ul></ul></div></div>";
        var operate = "<div class='toolbar toolbar" + random + "'><span class='info'><b>info: </b>" + rowNum + "行 x " + colNum + "列</span>" +
            "<span class='exportBnt' title='" + brhcTranslate("CROSS_TABLE.EXPORT") + "'></span></div>";
    ...
    bandDrillEvent: function (t_class, drill, render) {
        ...
        $.contextMenu({
            ...
            build: function ($trigger, e) {
                ...
                var items = {};
                if (up) {
                    items.up = {name: brhcTranslate("COMMON.ROLL_UP"), icon: "fa-arrow-up"}
                }
                if (down) {
                    items.down = {name: brhcTranslate("COMMON.DRILL_DOWN"), icon: "fa-arrow-down"}
                }
                return {
    ...
    renderPagination: function (pageCount, pageNumber, pageObj, target) {
        ...
        var liStr = '<li><a class="previewLink">' + brhcTranslate("CROSS_TABLE.PREVIOUS_PAGE") + '</a></li>';
        if (pageCount < 10) {
        ...
        liStr += '<li><a class="nextLink">' + brhcTranslate("CROSS_TABLE.NEXT_PAGE") + '</a></li>';
        if (target) {
